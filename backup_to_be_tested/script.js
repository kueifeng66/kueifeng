...
const dutySchedule = {
  ...
  "2024-5-22": "S: 黃榮國 A: 方振彬 N: 王金誠 C: 邱冠霖 R: 林厚運 T: 呂明峰",
  "2024-5-23": "S: 林森發 A: 羅應順 N: 許敦智 C: 秦桔萬 R: 張哲維 T: 許世勳",
  ...
  "2024-5-27": "S: 黃榮國 A: 彭偉慎 N: 王金誠 C: 秦桔萬 R: 黃煜森 T: 羅應順",
...
document.addEventListener('wheel', (event) => {
  ...
  // Calculate the next or previous day based on the scroll direction
  const nextDate = calculateNextDate(event.deltaY);

  // Show tooltip for the next or previous day
  showTooltip(nextDate);
  
  event.deltaY > 0 ? i++ : i--;

  // Ensure i stays within the range of days in the month
  if (i > daysInMonth) {
    i = daysInMonth;
  }
  if (i < 1) {
  ...
  if (!todayElement.textContent || !todayElement) {
    todayElement.style.backgroundColor="";
    return; 
  }
  // Remove highlight from previously highlighted day
  const previousHighlightedDay = document.querySelector('.highlighted');
  if (previousHighlightedDay) {
    previousHighlightedDay.classList.remove('highlighted');
  }
  var year = now.getFullYear();
  var month = (now.getMonth() + 1);
  var day = now.getDate();
  j = Zellercongruence(1,month,year) - 1;
  // Highlight the new day
  const dayElement = document.querySelector(`.day:nth-child(${i+j+day})`);
  if (dayElement) {
    dayElement.classList.add('highlighted');
  }
});

// Function to calculate the next or previous date based on the scroll direction
function calculateNextDate(deltaY) {
  const direction = deltaY > 0 ? 1 : -1;
  return `${year}-${month}-${i + direction}`;
}
