  ...
  fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${apiKey}`)
    ...
    .then(data => {
      ...
      if (weatherCondition === 'Clear') {
        document.body.style.background = 'url(sunny.png)';
      ...
      console.log('WeatherCodition:', weatherCondition);
    })
...
setInterval(fetchWeather, 60000); // 600000 milliseconds = 10 minutes


// temperature = temperature -  273.15;
const date2 = `${year}-${month}-${day}`;
let info = `${year}年${month}月${day}日` + holiday[date2] + " ☛☛☛ " + dutySchedule[date2];
scroll(info);
